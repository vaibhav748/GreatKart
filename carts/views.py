    ...
    if request.user.is_authenticated:
        ...
        if is_cart_item_exists:
            ...
            for item in cart_item:
                ...
                id.append(item.id)
            print("ex_variation------>", ex_var_list)
...
def remove_cart(request, product_id, cart_item_id):
    ...
    if request.user.is_authenticated:
        ...
        cart_item = CartItem.objects.filter(product=product, user=request.user, id=cart_item_id)[0]
        print(cart_item)
    else:
        cart = Cart.objects.get(cart_id=_cart_id(request))
        product = get_object_or_404(Product, id=product_id)
        cart_item = CartItem.objects.filter(product=product, cart=cart, id=cart_item_id)[0]
